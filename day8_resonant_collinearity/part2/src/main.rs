    ...
    for (_, positions) in antenna_positions {
        for idx1 in 0..positions.len() {
            for idx2 in (idx1 + 1)..positions.len() {
                let d_i = positions[idx1].0 - positions[idx2].0;
                let d_j = positions[idx1].1 - positions[idx2].1;

                let mut a_i = positions[idx1].0;
                let mut a_j = positions[idx1].1;

                // NOTE: We don't need to check if `a_i >= 0` because the type of usize will always
                // be non-negative.
                while a_i < n_row && a_j < n_col {
                ...
                let d_i = positions[idx2].0 - positions[idx1].0;
                let d_j = positions[idx2].1 - positions[idx1].1;

                let mut b_i = positions[idx2].0;
                let mut b_j = positions[idx2].1;

                while b_i < n_row && b_j < n_col {
